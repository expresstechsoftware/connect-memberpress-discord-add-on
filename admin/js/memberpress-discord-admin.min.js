...
if(response.previous_mapping){var mapjson=response.previous_mapping}else{var mapjson=localStorage.getItem('MemberPressMappingjson')}
...
var MemberPressMappingjson=jsonStart+'}';$("#maaping_json_val").html(MemberPressMappingjson);localStorage.setItem('MemberPressMappingjson',MemberPressMappingjson);draggable.css({'width':'100%','left':'0','top':'0','margin-bottom':'10px'})}
...
var MemberPressMappingjson=jsonStart+'}';localStorage.setItem('MemberPressMappingjson',MemberPressMappingjson);$("#maaping_json_val").html(MemberPressMappingjson)}
...
$('#ets_memberpress_btn_color').wpColorPicker();$('#ets_memberpress_discord_btn_disconnect_color').wpColorPicker();$(' .ets-memberpress-discord-review-notice > button.notice-dismiss').on('click',function(){$.ajax({type:"POST",dataType:"JSON",url:etsMemberpressParams.admin_ajax,data:{'action':'ets_memberpress_discord_notice_dismiss','ets_memberpress_discord_nonce':etsMemberpressParams.ets_memberpress_discord_nonce},beforeSend:function(){console.log('sending...')},success:function(response){console.log(response)},error:function(response){console.error(response)},complete:function(){}})});$('.serialized-data').on('click',function(){alert('ok');var data=$(this).data('content');$('.ets-log-popup').html('<pre>'+data+'</pre>').fadeIn()});$('.ets-log-popup').on('click',function(){$(this).fadeOut()})});$.skeletabs.setDefaults({keyboard:!1,})})(jQuery)